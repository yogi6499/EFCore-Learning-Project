...
RUN dotnet build "EFCore Learning Project.csproj" -c Release -o /app/build

FROM build AS publish
RUN dotnet publish "EFCore Learning Project.csproj" -c Release -o /app/publish /p:UseAppHost=false

FROM base AS final
WORKDIR /app
COPY --from=publish /app/publish .
ENTRYPOINT ["dotnet", "EFCore Learning Project.dll"]